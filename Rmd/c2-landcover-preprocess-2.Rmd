...
p_fig <- fp(p_root, "SAcropland/external/figures/")
p_data <- fp(p_root, "SAcropland/external/ext_data/")
```

Background data, projections, database connections, and SA grid
```{r, eval = FALSE}
# Get SA shape
if(!file.exists(fp(p_data, "MZshapes.Rdata"))) {
  system("wget http://dl.dropboxusercontent.com/u/31890709/MZshapes.Rdata")
}
load(fp(p_data, "MZshapes.Rdata"))
...
if(!file.exists(fp(p_data, "sagrid.tif"))) {  # already made? 
  ...
  projection(sa_r) <- sa_buf@proj4string
  sa_r <- writeRaster(sa_r, filename = fp(p_data, "sagrid.tif"), 
                      overwrite = TRUE)
} else {
...
```{r, eval = FALSE}
if(!file.exists(fp(p_data, "glcsa_masked.tif"))) {  # see if file exists,  
  ...
  glcsa_m <- raster(fp(p_data, "glcsa_masked.tif"))
}
