...
Let's start with the improved/fusion dataset, geo-wiki (note: GLC-Share was also initially tested, but it turned out to be identical to geo-wiki)
...
glc <- raster(fp(path, "glc_shv10_02.Tif"))
projection(glc) <- gcsstr
glcSA <- crop(glc, y=sa_buf_gcs, file = fp(path, "glcSA.tif"), 
...
glcSA_alb <- raster(fp(path, "glcSA_alb.tif"))

# Warp to SA grid
glcsa <- resample(glcSA_alb, sa_r, method = "bilinear", 
                  filename = fp(path, "glcSA_alb_rect.tif"))
glcsa_m <- mask(glcsa, sa_r, file = fp(p_data, "glcsa_masked.tif"),
                overwrite = TRUE)
...
```{r, eval = FALSE}
if(!file.exists(fp(p_data, "geowikisa_masked.tif"))) {  # see if file exists, to avoid redoing step 
   # first downloaded manually from password protected geo-wiki site
  # path <- "/u/lestes/spatial_data/geowiki/"
  path <- fp(p_dl, "geowiki/")
  unzip("cropland_hybrid_14052014v8.zip", exdir= "geowiki") 
  # geowiki <- raster(fp(path, "Hybrid_14052014V8.img"))
  geowiki <- raster(fp(path, "Hybrid_14052014V8.img"))
  # projection(geowiki) <- gcsstr
  # crs(geowiki) <- CRS(gcsstr)
  # geowikiSA <- crop(geowiki, y=sa_buf_gcs,
  #                   file = fp(path, "geowikiSA.tif"))  # crop to SA
  geowikiSA <- crop(geowiki, y=sa_buf_gcs, overwrite = TRUE, 
                    file = fp(p_dl, "geowiki/geowikiSA.tif"))  # crop to SA
  gdalwarp(srcfile = filename(geowikiSA), 
           dstfile = fp(path, "geowikiSA_alb.tif"), t_srs = prjstr, 
           tr = c(1000, 1000), r = "bilinear")
  geowikisa_alb <- raster(fp(path, "geowikiSA_alb.tif"))
  geowikisa <- resample(geowikisa_alb, sa_r, method = "bilinear", # Warp to SA
                        filename = fp(path, "geowikisa_alb_rect.tif"))    
  # geowikisa_m <- mask(glcsa, sa_r, 
  geowikisa_m <- mask(geowikisa, sa_r, 
                      file = fp(p_data, "geowikisa_masked.tif"), 
                      overwrite = TRUE)
} else {
  geowikisa_m <- raster(fp(p_data, "geowikisa_masked.tif"))
}

# test
tpath <- "croplandbias/external/ext_data/geowikisa_masked.tif"
geowikisa_mo <- raster(fp(p_root, tpath))
plot(geowikisa_mo - geowikisa_m)

```

And the standard products, Globcover 2009 and MODIS. Note the URL used originally has been superseded by the one commented out
```{r, eval = FALSE}
path <- "/u/lestes/spatial_data/globcover_2009/"
if(!file.exists(fp(path, "globSA_alb.tif"))) {
  url <- paste0("http://due.esrin.esa.int/globcover/LandCover2009/", 
  ...
  # glob <- raster(fp(path, "GLOBCOVER_L4_200901_200912_V2.3.tif"))
  glob <- raster(fp(p_dl,
                    "globcover_2009/GLOBCOVER_L4_200901_200912_V2.3.tif"))
  # globSA <- crop(glob, y=sa_buf_gcs)  # crop to extent of SA
  # e <- as(extent(c(-776722.9, 920077.1, -3831848, -2385248)),
  #                "SpatialPolygons")
  # crs(e) <- CRS(prjstr)
  # e <- spTransform(e, CRS(gcsstr))
  # globSA <- crop(glob, y = e, overwrite = TRUE,
  globSA <- crop(glob, y = sa_buf_gcs, overwrite = TRUE,
                 filename = fp(p_dl, "globcover_2009/globSA.tif"))  # crop
  # gdalwarp(srcfile = filename(globSA), overwrite = TRUE, 
  gdalwarp(srcfile = filename(globSA), overwrite = TRUE, 
           dstfile = fp(p_dl, "globSA_alb.tif"), t_srs = prjstr,
           # tap = TRUE,
           # dstfile = fp(path, "globSA_alb.tif"), t_srs = prjstr, 
           tr = c(300, 300))#, te = extent(e)[1:4])
  # globSA_alb <- raster(fp(path, "globSA_alb.tif"))
  globSA_alb <- raster(fp(p_dl, "globSA_alb.tif"))
} else {
  globSA_alb <- raster(fp(path, "globSA_alb.tif"))
}
...
moddir <- fp(p_dl, "MCD12Q1")
if(!file.exists(fp(moddir, "modis_type_1_alb.tif"))) {
  ...
  setwd(moddir)
  gdal_translate(src_dataset = modis[1], dst_dataset= "modis1.tif", 
                 sd_index= 1)
  r <- raster("modis1.tif")
  ...
  mod_1 <- raster("modis_type_1_alb.tif")
  file.remove(dir(moddir, pattern = "MCD12Q1"))
} else {
  mod_1 <- raster(fp(moddir, "modis_type_1_alb.tif"))
}
...
if(length(dir(p_data, pattern = "sa_.*._masked.tif")) != 2) {
  ...
  salc_l <- lapply(1:2, function(j) {
    ...
    nm <- fp(path, paste0(gsub("\\.tif", "", lcnm[j]), "_masked.tif"))
  ...
  salc_l <- lapply(lcmnm, function(x) raster(fp(p_data, x)))
} 
setwd(p_data)
```
...
### Mask out sugarcance
```{r, eval = FALSE}
if(!file.exists(fp(p_data, "kzn_cane_masked.tif"))) {
  path <- "/u/lestes/spatial_data/kzn_landcover/"
  setwd(kznpath)
  gdal_translate(src_dataset = "kznlc11v1w31", dst_dataset= "kznlandcover.tif",
                 of = "GTiff")
  system(paste("rm -r", fp(path, "kznlc11v1w31")))
  kznlc <- raster(fp(npath, "kznlandcover.tif"))
  ...
  kzn_cane <- raster(fp(path, kznlconm))
  # cext <- extent(c(20000, 40000, -3220000, -3200000))  # crops to small extent for examination
  # kznlc.ss <- crop(kznlc, cext, filename = fp(kznpath, "kznlandcover_crop.tif"), overwrite = TRUE)
  # plot(kznlc.ss)
  ...
  kzn_1km <- raster(fp(p_data, "kzn_cane_1km.tif"))
  kzn_1km_r <- resample(kzn_1km, sa_r, method = "bilinear")
  kzn_1km_m <- mask(kzn_1km_r, sa_r, 
                    filename = fp(p_data, "kzn_cane_masked.tif"), 
                    overwrite = TRUE)
} else {
  kzn_1km_m <- raster(fp(p_data, "kzn_cane_masked.tif"))
} 
...
```{r, eval = FALSE}
if(any(!file.exists(fp(p_data, 
                       dir(p_data, pattern = "mod_1*.*1kmrect_sum.tif"))))) {
  ...
  modsa_min <- assignLCPct(mod_1_cropb, minset, 
                           fname = fp(p_data, "mod_1_min.tif"))
  modsa_mu <- assignLCPct(mod_1_cropb, meanset, 
                          fname = fp(p_data, "mod_1_mu.tif"))
  modsa_max <- assignLCPct(mod_1_cropb, maxset, 
                           fname = fp(p_data, "mod_1_max.tif"))
  ...
  fnms <- fp(p_data, dir(p_data, "mod_1*.*1kmrect_sum.tif")[c(2:3, 1)])
  modsa_list_sum <- lapply(fnms, raster)
}  
setwd(p_data)
```
...
```{r, eval = FALSE}
if(any(!file.exists(fp(p_data, dir(p_data, "globSA*.*1kmrect_sum.tif"))))) {
  l <- lapply(c(11, 14, 20, 30), function(x) r <- globSA_alb == x)  # pull out classes
  s <- stack(l)
  globSA_cropb <- brick(s, filename = fp(p_data, "globSA_crop.grd"))
  rm(s, l)
  ...
  globsa_min <- assignLCPct(globSA_cropb, minset, 
                            fname = fp(p_data, "globSA_300_min.tif"))
  globsa_mu <- assignLCPct(globSA_cropb, meanset, 
                           fname = fp(p_data, "globSA_300_mu.tif"))
  globsa_max <- assignLCPct(globSA_cropb, maxset, 
                            fname = fp(p_data, "globSA_300_max.tif"))
  ...
  globsa_list_sum <- lapply(fp(p_data, dir(p_data, "globSA*.*1kmrect_sum.tif")[c(2:3, 1)]), raster)
} 
```
...
cover2007 <- brick(fp(p_data, "cover2007.tif"))
cover2011 <- brick(fp(p_data, "cover2011.tif"))
...
rm(m1, m2)
full_mask <- writeRaster(sa_masks + sust_mask, file = fp(p_data, "mask.tif"), overwrite = TRUE)
