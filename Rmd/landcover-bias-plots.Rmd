...
# DF <- do.call(rbind.data.frame, lapply(snms[-2], function(x) {
#   dstats_all[[2]][[x]]$f25[, c(1, 8)]
# }))
# dlm <- lm(mu ~ poly(V1, degree = 2), data = DF)
# plot(DF[, 1], DF[, 2])
# lines(predict.lm(dlm, newdata = data.frame("V1" = 1:100)))
...
#         highcounter[j] <- 1
#         xi <- toohigh[which.max(d[toohigh, "mu"])]
#         text(xi, y = pp$yl2[[bt]][2], labels = round(d[xi, "mu"]), 
#              col = "grey50", xpd = NA)
#       }
#       if(j == length(lev)) {
#         if(sum(highcounter) > 0) {
#           mtext("max", side = 3, cex = 0.7, col = "grey50", line = -0.7, 
#                 adj = 1.1)
#         }
#       }
#       if(j == 1) {
...
#       axis(1, at = c(1, 5, 10, 15, 20), labels = rep("", 5), mgp = c(1, 1, 0), 
#            tcl = -0.7, lwd.ticks = 1.2, pos = pp$yl2[[bt]][1])
#       axis(1, at = 1:20, labels = rep("", 20), las = 2, mgp = c(1, 0.4, 0), 
