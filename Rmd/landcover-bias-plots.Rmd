...
dev.off()
```
The map below was created by the code above, showing biases at 1 km, 25 km, 50 km, 
and 100 km from South Africa's landcover product, the mean estimates from 
...
pp <- list("x" = c(-2, 21), "y" = c(0, 0), 
           ...
           "yl" = list(c(-100, 100), c(0, 100)), "yl2" = list(c(-40, 40), c(0, 60)), 
           "xl" = c(1, 20),
...
for(bt in 1:2) {
  ...
  for(i in 1:length(snms)) {
    ...
    if(i == 1) axis(2, at = pp$yax1[[bt]], labels = pp$yax1[[bt]], las = 2, 
                    mgp = c(1, 0.6, 0))
    axis(1, at = 1:20, labels = pp$xaxl, las = 2, mgp = c(1, 0.4, 0), tcl = -0.3, 
         pos = pp$yaxp[[bt]], cex = 0.8)
    for(z in 1:length(pp$y2[[bt]])) lines(pp$x, rep(pp$y2[[bt]][z], 2), 
                                          lwd = pp$y2lwd, col = pp$y2lc[[bt]][z])
    for(k in 1:n) {
      a <- seq(grep("2.5", colnames(d)), by = 1, length.out = 6)
      if(!is.na(d[k, 2])) boxplot_v(k, d[k, a], n, 150, 10, bcol, fcol, pcex = 0.5, 
                                    lwd = c(1, 0.5, 2))
    }
  ...
  for(i in 1:length(snms)) {
    ...
    for(j in 1:length(lev)) {
      plot(pp$x, pp$y, ylim = pp$yl2[[bt]], xlim = pp$xl, type = "l", lwd = 1, 
      ...
      if(j == length(lev)) {
        mtext(expression(mu), side = 1, cex = 0.8, col = "grey10", 
              line = ifelse(bt == 1, -1.5, -1.8), adj = 1.1)
      }
      ...
      if(length(toohigh) > 0) {
        ...
        text(xi, y = pp$yl2[[bt]][2], labels = round(d[xi, "mu"]), col = "grey50", 
             xpd = NA)
      }
      if(j == length(lev)) {
        if(sum(highcounter) > 0) mtext("max", side = 3, cex = 0.7, col = "grey50", 
                                       line = -0.7, adj = 1.1)
      }
      if(j == 1) {
        axis(2, at = seq(pp$yl2[[bt]][1], pp$yl2[[bt]][2], 10), 
             labels = pp$yax2[[bt]], las = 2, mgp = c(1, 0.6, 0))
      }
      if(i == 1) mtext(levnms[j], side = 3, line = -0.4, cex = 0.8)
      axis(1, at = c(1, 5, 10, 15, 20), labels = rep("", 5), mgp = c(1, 1, 0), 
           tcl = -0.7, lwd.ticks = 1.2, 
           pos = pp$yl2[[bt]][1])
      axis(1, at = 1:20, labels = rep("", 20), las = 2, mgp = c(1, 0.4, 0), 
           tcl = -0.3, pos = pp$yl2[[bt]][1])
      if(i == length(snms)) {
  ...
  mtext(ifelse(bt == 1, "% bias", "% bias (absolute)"), side = 2, line = 2.2, 
        cex = 0.8, outer = TRUE, adj = 0.35)
