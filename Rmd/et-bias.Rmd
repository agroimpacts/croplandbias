...
yrs <- gsub("*.*_", "", lnames)
mos <- gsub("_.*", "", lnames)
yi <- t(sapply(unique(yrs), function(x) range(which(yrs == x))))
mi <- sapply(unique(mos), function(x) which(mos == x))
```
...
par(mfcol = c(4, 5), mar = c(0, 0, 0, 0), oma = c(5, 5, 2, 0))
...
flex_legend(ncuts = length(brks) - 1, legend.text = legtext[i], 
            ...
            legend.pos = c(4, 5), leg.adj = list(c(0.1, 0.3), c(-1, -0.5)), 
...
### Mean percent bias in ET estimates
```{r, eval=FALSE}
lev_vec <- "f25"
...
mus[, ol := NULL]
setnames(mus, c("x", "il"), c("ETvar", "dataset"))
setkeyv(mus, c("dataset", "ETvar"))
muas <- rbindlist(lapply(names(mua), function(x) cbind(x, mua[[x]])))
muas[, ol := NULL]
setnames(muas, c("x", "il", "bias"), c("ETvar", "dataset", "absbias"))
setkeyv(muas, c("dataset", "ETvar"))
mutab <- mus[muas]
mutab[, lapply(.SD, function(x) max(abs(x))), .SDcols = 3:4]

mutab_xtab <- xtable::xtable(mutab, digits = 1)
print(mutab_xtab, type = "latex", 
      file = "paper/figures/et-bias.tex")
```
<a href="#top">Back to top</a>

The largest (in an absolute sense) mean actual bias is -0.6%, while the largest mean absolute bias is 1.43%, so not much here.
