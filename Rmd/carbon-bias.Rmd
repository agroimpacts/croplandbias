...
# bias estimates
tot_area <- sum(freq(namask)[1:2, ])
crop_areas <- lapply(lc_union$f1, freq)
sapply(crop_areas, function(x) x[2, 2] / tot_area)  # 29, 31, 33, 53 (Glob)%
...
pdf(full_path(p_fig, "carbon_veg_scale3.pdf"), height = 7, width = 7)
...
mu0s <- copy(rbind(cbind(Bias = "Actual", mu0), 
                   cbind(Bias = "Absolute", mu0a)))
for(i in 1:length(LC)) mu0s[V2 == LC[i], V2 := pnms[i]]
for(i in 1:length(snms)) mu0s[V1 == snms[i], V1 := mcap[i]]
setnames(mu0s, c("V1", "V2", lev_vec), 
         c("Map", "Cover", paste(c(1, fact), "km")))
caption <- paste("Mean actual and absolute biases for each of the cropland", 
...
mu0s_xtab <- xtable(mu0s, digits = 1, caption = caption)
...
mus <- copy(rbind(cbind(Bias = "Actual", mu), 
                  cbind(Bias = "Absolute", mua)))
for(i in 1:length(LC)) mus[V2 == LC[i], V2 := pnms[i]]
for(i in 1:length(snms)) mus[V1 == snms[i], V1 := mcap[i]]
...
mus_xtab <- xtable::xtable(mus, digits = 1, caption = caption)
