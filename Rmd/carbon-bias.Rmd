...
### Extract statistics for supplemental version 
```{r, eval=FALSE}
# cropland area only
...
### Plot mean carbon bias and accuracy against scale

#### Bias and accuracy weighted by agricultural area 
...
legend(x = 12.4, y = -210, legend = lcnms, pch = 15, col = cols, adj = 0,
       pt.cex = 1.5, bty = "n", cex = 0.8, x.intersp = 0.5)
legend(x = 11.9, y = -210, legend = rep("", 4), pch = 15, adj = 0, 
       col = makeTransparent(cols, alpha = alph[2]), pt.cex = 1.5, bty = "n", 
       cex = 0.8)
text(x = 12.7, y = -212, labels = "MAE", srt = 45, adj = c(0, 0), cex = 0.8)
text(x = 12.2, y = -212, labels = "Bias", srt = 45, adj = c(0, 0), cex = 0.8)
legend(x = 12.1, y = 260, legend = c("Forest", "Shrubland", "Sparse", "Mean"), 
...
#### Agricultural area bias and accuracy
...
#### Density independent bias and accuracy

```{r, eval=FALSE}
yl <- range(mund[, 3:8, with = FALSE], munda[, 3:8, with = FALSE])
yl <- round(yl / 10) * 10
yax <- seq(yl[1], yl[2], 10)
xax <- seq(1, 6, 5 / (length(yax) - 1))
...
pdf(full_path(p_fig, "carbon_veg_scale_nd.pdf"), height = 7, width = 7)
par(mar = rep(1, 4), oma = c(2, 2, 0, 0), mgp = c(1, 0.5, 0), tcl = -0.3)
plot(xl, yl, pch = "", yaxt = "n", xaxt = "n", xaxs = "i", yaxs = "i", 
     ylab = "", xlab = "")#,
for(i in shd) polyfunc2(x = i, y = yl, w = 3, col = g1, bcol = g1, lwd = 1)
polyfunc2(x = 8.75, y = yl, w = 18.5, col = "transparent", bcol = "black")
grid(NA, length(yax) - 1, lwd = 1, lty = 1)
lines(c(-1, 18), c(0, 0), lwd = 2, col = "grey50")
for(i in 1:length(lev_vec)) {
  for(j in 1:length(snms)) {
    for(k in 1:length(mutype)) {
...
mtext(side = 1, text = "Resolution (km)", outer = TRUE, line = 0.5)
mtext(side = 2, text = "Bias/MAE (%)", outer = TRUE, line = 1)
legend(x = 12.4, y = yll, legend = lcnms, pch = 15, col = cols, adj = 0,
       pt.cex = 1.5, bty = "n", cex = 0.8, x.intersp = 0.5)
legend(x = 11.9, y = yll, legend = rep("", 4), pch = 15, adj = 0, 
       col = makeTransparent(cols, alpha = alph[2]), pt.cex = 1.5, bty = "n", 
       cex = 0.8)
text(x = 12.7, y = yll - 2, labels = "MAE", srt = 45, adj = c(0, 0), cex = 0.8)
text(x = 12.2, y = yll - 2, labels = "Bias", srt = 45, adj = c(0, 0), cex = 0.8)
legend(x = 12.1, y = 165, legend = c("Forest", "Shrubland", "Sparse", "Mean"), 
       pch = pchs, pt.cex = c(2, 1.5, 1.5, 2), bty = "n", cex = 0.8)
dev.off()
...
# still to do: tables for density independent biases

# Bind all three tables together
fulls <- rbind(cbind("Region" = "Density weighted", muws), 
               cbind("Region" = "Country", mu0s),
               cbind("Region" = "Agricultural", mus)) 

caption <- paste("Biases and mean absolute errors for each of the cropland", 
                 "maps across aggregation scales and each possible landcover", 
                 "type sharing the pixel with cropland. Three variants", 
                 "of the bias/accuracy values are provided, indicated by the:",
                 "Region column: 1) weighted by reference cropland density", 
                 "(Density weighted); 2) across the entire country (Country);",
                 "3) for agricultural areas only (Agricultural, i.e.", 
                 "$>$0.05\\% cropland).")
