...
cb_stats <- lapply(lev_vec, function(x) {  # level
  ...
  l1 <- lapply(names(c_pct_diff[[x]]), function(y) {
    ...
    DT <- na.omit(agroEcoTradeoff::as.data.table.raster(s))
    setkey(DT, "bin")
    # indexing columns
...
extract_stat <- function(namevec, stats, i1, i2) {
  estats <- do.call(rbindlist(lapply(namevec, function(i) {
    data.table(sapply(stats, function(x) {
...
```{r, eval=FALSE}
a <- extract_stat(LC, cb_stats, "all", "m")  # mean abs crop only
b <- cbind(V2 = "all", a[, lapply(.SD, mean), by = V1, .SDcols = lev_vec])
setcolorder(a, c(2:1, 3:ncol(a)))
mu <- rbind(a, b)

a <- extract_stat(LC, cb_stats, "all", "ma")  # mean abs crop only
b <- cbind(V2 = "all", a[, lapply(.SD, mean), by = V1, .SDcols = lev_vec])
setcolorder(b, c(2:1, 3:ncol(b)))
mua <- rbind(a, b)

a <- extract_stat(LC, cb_stats, "all", "m0")  # mean abs crop only
b <- cbind(V2 = "all", a[, lapply(.SD, mean), by = V1, .SDcols = lev_vec])
setcolorder(b, c(2:1, 3:ncol(b)))
mu0 <- rbind(a, b)

a <- extract_stat(LC, cb_stats, "all", "ma0")  # mean abs crop only
b <- cbind(V2 = "all", a[, lapply(.SD, mean), by = V1, .SDcols = lev_vec])
setcolorder(b, c(2:1, 3:ncol(b)))
mu0a <- rbind(a, b)
...
for(f in 1:length(LC)) {
  ...
  for(i in 1:4) {
    lines(1:6, mu[V1 == snms[i] & V2 == LC[f], lev_vec, with = FALSE], 
          col = cols[i], lty = 2, lwd = lw)
  }
  for(i in 1:4) {
    lines(1:6, mua[V1 == snms[i] & V2 == LC[f], lev_vec, with = FALSE], 
...
plot(xax, yax, pch = "", yaxt = "n", xaxt = "n", xaxs = "i", yaxs = "i", 
     ...
     panel.first = grid(NA, length(yax), lwd = 1, lty = 1))
for(i in 1:4) {
 lines(1:6, all_mu[V1 == snms[i], lev_vec, with = FALSE], col = cols[i], 
       lty = 2, lwd = lw)
}
for(i in 1:4) {
  lines(1:6, all_mua[V1 == snms[i], lev_vec, with = FALSE], col = cols[i],
        lty = 1, lwd = lw)
}
...
mtext(side = 2, text = "% bias", outer = TRUE, line = 2)
par(xpd = NA)
x <- grconvertX(0.7, from = "ndc", to = "user")
y <- grconvertY(0.075, from = "ndc", to = "user")
legend(x = x, y = y, legend = c("actual bias", "absolute bias"), lty = c(2, 1), 
       lwd = 2, bty = "n", cex = 1.2)
...
nmrepl <- cbind(snms, lcnms)
mu0[, c(lev_vec) := lapply(.SD, round), .SDcols = lev_vec]
mu0a[, c(lev_vec) := lapply(.SD, round), .SDcols = lev_vec]
mu0[, V1 := nmrepl[match(V1, nmrepl), 2]]
mu0a[, V1 := nmrepl[match(V1, nmrepl), 2]]
mu0s <- rbind(cbind(stat = "actual", mu0), cbind(stat = "absolute", mu0a))
mu0s_xtab <- xtable::xtable(mu0s)
...
mu[, V1 := nmrepl[match(V1, nmrepl), 2]]
mua[, V1 := nmrepl[match(V1, nmrepl), 2]]
mus <- rbind(cbind(stat = "actual", mu), cbind(stat = "absolute", mua))
mus_xtab <- xtable::xtable(mus)
print(mu0s_xtab, type = "latex", 
...
pdf(full_path(p_fig, "carbon_veg_scale2.pdf"), height = 7, width = 7)
