...
# snms <- names(p_diff1)
# lcnu <- lapply(lev, function(x) {
#   lcb <- lapply(snms, function(y) {
#     all_gt0 <- !is.na(bins[[x]])
#     all_gt0
#   })
#   named_out(lcb, snms)
# })
...
a <- bias_stats_list(bins, awgts, lcu, cf_pct_diff2, wm, "mu", "bias", TRUE)
b <- bias_stats_list(bins, awgts, lcu, cf_pct_diff2, wma, "mua", "bias", TRUE)
# b <- bias_stats_list(bins, awgts, lcnu, cf_pct_diff2, wma, "mua", "bias", TRUE)
# b[ol == "f1" & il == "modmu" & bin == "all"]
...
lcfmua <- extract_stat(lev, snms, "all", "mua", "bias", b)
lcfmua_nm <- extract_stat(lev, snms, "all", "mua.nm", "bias", b) 
# b[ol == "f10" & il == "globmu" & bin == "all"]
lcfmua_nd <- extract_stat(lev, snms, 1:20, "mua.nm", "bias", b, 
                          type = "nodensity")
b[ol == "f1" & il == "sa30" & bin != "all" & bvals == "mua.nm", mean(bias)]
...
svec <- c("mua.nm", "mua")
aa <- c("Country", "Agricultural")
mcap <- c("SA-LC", "GlobCover", "MODIS", "GeoWiki")

lcf_out <- do.call(rbind, lapply(1:length(svec), function(i) {
  a <- do.call(rbind, lapply(snms, function(x) {
    aa <- sapply(lev, function(y) {
      b[ol == y & il == x & bin == "all" & bvals == svec[i], bias]
    })
    named_out(aa, paste(c(1, fact), "km"))
  }))
  cbind.data.frame("Region" = aa[i], "Map" = mcap, a)
}))

caption <- paste("Mean absolute errors (MAE) in statistically constrained", 
                 "cropland maps, calculated over the entire country, and for", 
                 "the union of agricultural regions (cropland $>$ 0) defined ", 
                 "by the reference and corresponding adjusted cropland",
                 "maps")    
lcfout_xtab <- xtable(lcf_out, digits = 1, caption = caption)
print(lcfout_xtab, type = "latex", file = "paper/figures/cropadj-mae.tex", 
...
muy <- sapply(lev, function(i) {
  ...
  v
})  # yields across scales and datasets all 3.4-3.5 kg/ha
...
# type 2 (non-zero)
# list2 <- lapply(cyld_agg3, function(x) x[-1]) # cyld_agg for a, b/c correct
...
fnms <- c("prod_bias_map_1.pdf", "yld_bias_map_1.pdf")  # output names
...
From the above it is clear that the nature in which aggregation of yields are done has some impact on both yield and production estimates. There are three types of yield aggregation that can be done (most commented out now after initial checking): 
...
Only Types 1 and A are included in further investigation. The other variants are left as accuracy checks.  
...
# p1sm <- extract_stat(lev, snms, "all", "sum", "bias", pd1_st)  # ident to p3
...
pdf(full_path(p_fig, "yield_bias_scale.pdf"), height = 7, width = 7)
...
lapply(1:length(lev), function(ii) {
  ...
  lapply(1:length(snms), function(i) {
    nm <- snms[i]
    lapply(1:length(malmul), function(k) {
      mm <- malmul[[k]]
      v <- sapply(1:length(mm), function(j) {
        round(get(mm[j])[ol == lv & il == nm,  bias] / get(pyl[j])[lv] * 100,1)
      })
...
aa <- c("Bias", "MAE")
out_tab <- do.call(rbind, lapply(1:2, function(x) {
...
caption <- paste("Biases and mean absolute errors (MAE) in disaggregated maize",
                 ...
                 "maps. Bias and MAE were normalized to their", 
