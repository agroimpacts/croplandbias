...
# legend(x = 12.1, y = 65, legend = c("Prod I", "Prod II", "Yield 1", "Yield 2"),        pch = pchs, pt.cex = c(2.5, 1.5, 1.5, 1.5), bty = "n", cex = 0.8)
...
out_tab <- do.call(rbind, lapply(1:2, function(x) {
  ...
  colnames(out) <- c("Bias", "Map", "Variable", paste(c(1, fact), "km"))
  out
}))  

caption <- paste("Mean actual and absolute biases for yield and", 
                 "production estimates disaggregated from magisterial", 
                 "district level farm census data onto adjusted cropland", 
                 "maps. Biases were calculated as percent differences", 
                 "relative to the mean value calculated from the reference", 
                 "map.")    
