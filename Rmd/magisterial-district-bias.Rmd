---
title: "District-level cropcover and bias"
author: "Lyndon Estes"
date: "June 4, 2015"
output: 
  html_document:
...
## Bias in relation to typical crop cover
...
paths <- full_path(p_data, dir(p_data, paste0("cover*.*11sum_mask*")))
...
1. Extract the cropland percentages for magisterial district, as well as the absolute biases. 
```{r, eval=FALSE}
# GTI data
# mean cropland cover
...
# how many contribute when non-cropland removed
...
2. Run regressions and check for spatial autocorrelation
```{r, eval=FALSE}
...
3. A generalized least squares (GLS) with spatial autocovariance terms. 
...
plot(fit_distrs[[1]]$lnorm)  # lognorm shows up as best fit

# Matters more on residuals from model fit, however. 
# dataset
# dat <- proc_df(x = mu0, nm = snms[4])  # interactively change response  
...
dgam4b <- gam(log(dv) ~ s(pv, k = 3) + s(x, y), weights = w, data = dat, 
             method = "ML")
summary(dgam4b)
AIC(dgam4b)
gam.check(dgam4)
...
lw <- c(rep(2, 4), 3, 3)
yl1 <- c(0, round(max(sapply(gam_mods, function(x) max(x$pdat1$fit))), 1))
yl2 <- c(0, round(max(sapply(gam_mods, function(x) max(x$pdat2$fit))), 1))
pdf(full_path(p_fig, "biases_md_lnorm_gam_mu0.pdf"), width = 7, height = 7)
par(oma = c(0, 0, 0, 0), mar = c(3, 3, 2, 2), mgp = c(1.5, 0.5, 0))
plot(xl, yl2, pch = "", ylab = "% bias", xlab = "% cropland", xaxs = "i",
     mgp = c(1.5, 0.5, 0), yaxs = "i", yaxt = "n")
...
pdf(full_path(p_fig, "biases_md_lnorm_gam_med0.pdf"), width = 7, height = 7)
...
Final plot of magisterial districts, for supplementals
```{r, eval=FALSE}
