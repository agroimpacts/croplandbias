...
load(full_path(p_data, "diff_grid_abs.rda"))  # actual diffence grids
...
cx <- 1.5
lcol <- "black"
mcap <- c("SA-LC", "GlobCover", "MODIS", "GLC-Share", "GeoWiki")
snms <- c("sa30", "globmu", "modmu", "glcsh", "geow")
agglev <- c("f1", "f20", "f50")
agglev2 <- c("1 km", "20 km", "50 km")
pdf(full_path(p_fig, "bias_map.pdf"), height = 12, width = 8)
par(mfrow = c(5, 3), mar = c(0, 0, 0, 0), oma = c(7, 5, 2, 0))
for(i in 1:length(snms)) {
  for(j in 1:length(agglev)) {
    plot(sa.shp, lty = 0)
    plot(diff_list_act$d2011[[agglev[j]]][[snms[i]]], add = TRUE, col = cols(n_cols), breaks = brks, 
         legend = FALSE)
