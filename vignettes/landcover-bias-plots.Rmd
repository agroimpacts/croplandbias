...
dev.off()
```
The map below was created by the code above, showing biases at 1 km, 20 km, 50 km, and 100 km from South Africa's landcover product, the mean estimates from GlobCover and MODIS, and Geowiki/GLC-SHARE.  

<p align="center"><img src="../external/figures/bias_map.pdf" width="400px"></p> 
...
pp <- list("x" = c(-2, 21), "y" = c(0, 0), 
           ...
           "y2" = list(c(seq(-100, -10, 10), seq(10, 100, 10)), seq(10, 100, 10)), 
           "y1lwd" = list(2, 1),  
           "y2lwd" = 0.5, #c(1, 0.5, 1, 0.5, 0.5, 0.5, 0.5, 1, 0.5, 1),
           "y2lc" = list(c("black", rep("grey80", 4), "black", rep("grey80", 4), 
                           rep("grey80", 4), "black", rep("grey80", 4), "black"),  rep("grey80", 10)),
           "y3lwd" = rep(2, length(lev)), 
           "yl" = list(c(-100, 100), c(0, 100)), "yl2" = list(c(-40, 40), c(0, 60)), "xl" = c(1, 20),
           ...
           "yax2" = list(unlist(lapply(seq(-40, 40, 20), function(x) c(" ", x)))[-1], 
...
for(bt in 1:2) {
  pdf(full_path(p_fig, btype[bt]), width = 7, height = 7)
  lmat <- rbind(m, m3)
  l <- layout(lmat)
  par(mar = c(3, 0, 0, 0), oma = c(3, 4, 2, 2))
  ...
  for(i in 1:length(snms)) {
    ...
    d <- dset[[snms[i]]]$f1
    n <- nrow(d)
    plot(pp$x, pp$y, ylim = pp$yl[[bt]], xlim = pp$xl, type = "l", lwd = pp$y1lwd[[bt]], axes = FALSE, 
         xlab = "", ylab = "")
    if(i == 1) axis(2, at = pp$yax1[[bt]], labels = pp$yax1[[bt]], las = 2, mgp = c(1, 0.6, 0))
    axis(1, at = 1:20, labels = pp$xaxl, las = 2, mgp = c(1, 0.4, 0), tcl = -0.3, pos = pp$yaxp[[bt]], 
         cex = 0.8)
    for(z in 1:length(pp$y2[[bt]])) lines(pp$x, rep(pp$y2[[bt]][z], 2), lwd = pp$y2lwd, col = pp$y2lc[[bt]][z])
    for(k in 1:n) {
    ...
    if(i == 1) mtext(ifelse(bt == 1, "% bias", "% bias (absolute)"), side = 2, line = 2.2, cex = 0.8)
    mtext(lcnms[i], side = 3, line = 0, col = cols[i])
  }
  mtext("% cropland", side = 1, outer = TRUE, line = -36.7, cex = 0.8)
  par(mar = c(0.5, 0, 0.5, 0.3))
  for(i in 1:length(snms)) {
    highcounter <- rep(0, length(lev))
    for(j in 1:length(lev)) {
      plot(pp$x, pp$y, ylim = pp$yl2[[bt]], xlim = pp$xl, type = "l", lwd = 1, axes = FALSE, xlab = "", 
           ylab = "", xaxs = "i")
      ...
      muv <- round(mean(d[, 8], na.rm = TRUE))
      if(j == length(lev)) {
        mtext(expression(mu), side = 1, cex = 0.8, col = "grey10", line = ifelse(bt == 1, -1.5, -1.8), 
              adj = 1.1)
      }
      text(10, pp$yl2[[bt]][1] + 10, labels = muv, cex = 1.3, col = "grey10")
      toohigh <- which(d[, "mu"] > pp$yl2[[bt]][2])
      if(length(toohigh) > 0) {
        highcounter[j] <- 1
        xi <- toohigh[which.max(d[toohigh, "mu"])]
        text(xi, y = pp$yl2[[bt]][2], labels = round(d[xi, "mu"]), col = "grey50", xpd = NA)
      }
      if(j == length(lev)) {
        if(sum(highcounter) > 0) mtext("max", side = 3, cex = 0.7, col = "grey50", line = -0.7, adj = 1.1)
      }
      if(j == 1) {
        axis(2, at = seq(pp$yl2[[bt]][1], pp$yl2[[bt]][2], 10), labels = pp$yax2[[bt]], las = 2, 
             mgp = c(1, 0.6, 0))
      }
      if(i == 1) mtext(levnms[j], side = 3, line = -0.4, cex = 0.8)
      axis(1, at = c(1, 5, 10, 15, 20), labels = rep("", 5), mgp = c(1, 1, 0), tcl = -0.7, lwd.ticks = 1.2, 
           pos = pp$yl2[[bt]][1])
      axis(1, at = 1:20, labels = rep("", 20), las = 2, mgp = c(1, 0.4, 0), tcl = -0.3, pos = pp$yl2[[bt]][1])
      if(i == length(snms)) {
        axis(1, at = 1:20, labels = pp$xaxl2, las = 2, mgp = c(1, 0.8, 0), tcl = -0.3, pos = pp$yl2[[bt]][1])
      }
      }
    }
  mtext(ifelse(bt == 1, "% bias", "% bias (absolute)"), side = 2, line = 2.2, cex = 0.8, outer = TRUE, 
        adj = 0.35)
