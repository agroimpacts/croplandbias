...
for(i in 1:length(snms)) {
  ...
  if(i == 1) axis(2, at = seq(-100, 100, 50), labels = seq(-100, 100, 50), las = 2, mgp = c(1, 0.6, 0))
  axis(1, at = 1:20, labels = pp$xaxl, las = 2, mgp = c(1, 0.4, 0), tcl = -0.3, pos = -100, cex = 0.8)
  ...
  if(i == 1) mtext("% bias", side = 2, line = 2.2, cex = 0.8)
...
for(i in 1:length(snms)) {
  ...
  for(j in 1:length(lev)) {
    ...
    #pval <- substitute(mu==k, list(k = muv))
    #if(j == 1) text(10, pp$yl2[1] + 10, labels = pval, cex = 1.3, col = cols[i])
    if(j == length(lev)) {
      mtext(expression(mu), side = 1, cex = 0.8, col = cols[i], line = -1.5, adj = 1.1)
    }
    text(10, pp$yl2[1] + 10, labels = muv, cex = 1.3, col = cols[i])
    toohigh <- which(d[, "mu"] > pp$yl2[2])
    if(length(toohigh) > 0) {
      highcounter[j] <- 1
      xi <- toohigh[which.max(d[toohigh, "mu"])]
      text(xi, y = pp$yl2[2], labels = round(d[xi, "mu"]), col = cols[i], xpd = NA)
    }
    if(j == length(lev)) {
      if(sum(highcounter) > 0) mtext("max", side = 3, cex = 0.7, col = cols[i], line = -0.7, adj = 1.1)
    }
    #if(j == 1) axis(2, at = pp$yax2, labels = pp$yax2, las = 2)
    if(j == 1) axis(2, at = seq(pp$yl2[1], pp$yl2[2], 10), labels = pp$yax2, las = 2, mgp = c(1, 0.6, 0))
    if(i == 1) mtext(levnms[j], side = 3, line = -0.4, cex = 0.8)
    axis(1, at = c(1, 5, 10, 15, 20), labels = rep("", 5), mgp = c(1, 1, 0), tcl = -0.7, lwd.ticks = 1.2, 
         pos = pp$yl2[1])
    axis(1, at = 1:20, labels = rep("", 20), las = 2, mgp = c(1, 0.4, 0), tcl = -0.3, pos = pp$yl2[1])
    if(i == length(snms)) {
      axis(1, at = 1:20, labels = pp$xaxl2, las = 2, mgp = c(1, 0.8, 0), tcl = -0.3, pos = pp$yl2[1])
    }
  }
}
mtext("% bias", side = 2, line = 2.2, cex = 0.8, outer = TRUE, adj = 0.35)
