...
# if(!file.exists(full_path(p_data, "salc_ag_masked.tif"))) {
  ...
  nm <- paste(gsub("\\.tif", "", lcnm[j]), "_1km.tif", sep = "")
  ...
  gdalwarp(srcfile = lcnm[j], t_srs = projection(sa_r), dstfile = nm, r = "average", ot = "Float32", 
           ...
           overwrite = TRUE)
  #gdal_calc(cstr = "A*B", x = list("A" = nm, "B" = sa_mask), filename = "sa_ag_masked.tif")
  
  r <- raster("sa_ag_1km.tif")
  plot(r)
