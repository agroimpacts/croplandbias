...
# now compare to originally made glc-share
# r2 <- raster(fp(p_orig, "upstream/glcsa_masked.tif"))
# plot(r - r2)
# cellStats(abs(r - r2), sum)  # identical, hallejulah
```
...
# now test against original globSA_alb.tif
# r2 <- raster(fp(p_orig, "upstream/globSA_alb.tif"))
# rdif <- r - r2
# rdif <- Which(rdif != 0)
# rdifnna <- Which(!is.na(rdif))
# cellStats(rdif, sum)  # differences at 1117 pixels
# cellStats(rdifnna, sum)  # out of 27273232 pixels
```
...
# r2 <- lapply(sapply(r, names), function(x) {
#   raster(fp(p_orig, paste0("upstream", x, ".tif")))
# })
# rdif <- lapply(1:2, function(x) r[[x]] - r2[[x]])
# rdif <- lapply(rdif, function(x) Which(x != 0))
# rdifnna <- lapply(rdif, function(x) Which(!is.na(x)))
# lapply(rdif, function(x) cellStats(x, sum))  # 0
# identical
```
...
gdalwarp(srcfile = filename(r), t_srs = projection(sa_r), 
         dstfile = fp(path, "kzn_cane_1km.tif"), r = "average", ot = "Float32",          srcnodata = 255, tr = c(1000, 1000), of = "GTiff", 
...
# check against original mask
# r2 <- raster(fp(p_orig, "upstream/kzn_cane_masked.tif"))
# rdif <- round(r - r2, 7)  # needs rounding otherwise inferno
# rdif <- Which(rdif != 0)
# rdifnna <- Which(!is.na(rdif))
# cellStats(rdif, sum)  # 0 differences
# cellStats(rdifnna, sum)  # out of 10668451 pixels
# plot(rdif)
# identical, with rounding
...
# check against old modmu
# r2 <- raster(fp(p_orig, "downstream/mod_1_mu_1kmrect_sum.tif"))
# rdif <- r[[2]] - r2  # needs rounding otherwise inferno
# rdif <- Which(rdif != 0)
# rdifnna <- Which(!is.na(rdif))
# cellStats(rdif, sum)  # 0 differences
# cellStats(rdifnna, sum)  # out of 10668451 pixels
...
# check against old globSA mu
# r2 <- raster(fp(p_orig, "downstream/globSA_mu_1kmrect_sum.tif"))
# rdif <- round(r[[2]] - r2, 7)  # needs rounding otherwise inferno
# hist(rdif, breaks = seq(-7, 7, 0.1))
# rdif <- Which(rdif != 0)
# rdifnna <- Which(!is.na(rdif))
# cellStats(rdif, sum)  # 1518 cells different
# cellStats(rdifnna, sum)  # out of 2276736 pixels
```
...
r <- lapply(list(cover2007, cover2011), function(y) {
  r <- calc(y[[-6]], sumfun)
  r[r > 100] <- 100
  writeRaster(r, file = gsub("\\.tif", "sum.tif", y@file@name),
              overwrite = TRUE)
})
...
m1[is.na(m1)] <- 0
m1[m1 > 0] <- NA
m2 <- raster(fp(p_dl, "sa_lc_2009/sa_af_masked.tif"))
m2[is.na(m2)] <- 0
m2[m2 > 0] <- NA
sa_masks <- sa_r > 0
...
# test 
# r2 <- lapply(r, function(x) {
#   raster(fp(p_orig, paste0("downstream/", basename(filename(x)))))
# })
# rdif <- lapply(1:2, function(x) r[[x]] - r2[[x]])
# rdif <- lapply(rdif, function(x) Which(x != 0))
# rdifnna <- lapply(rdif, function(x) Which(!is.na(x)))
# lapply(rdif, function(x) cellStats(x, sum))  # 0
# lapply(rdifnna, function(x) cellStats(x, sum))  # 2276736
# identical
