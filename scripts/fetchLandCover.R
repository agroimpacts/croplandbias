...
setwd(p.alyze)
if(any(!file.exists(dir(p.alyze, pattern = "mod_1*.*1kmrect_sum.tif")))) {
  ...
  modsa_list <- lapply(dirPath(p.alyze, dir(p.alyze, pattern = "mod_1*.*1kmrect")), raster)
  ...
  modsa_list_sum <- lapply(dir(p.alyze, "mod_1+.+1kmrect_sum.tif")[c(2:3, 1)], raster)
}  

if(any(!file.exists(dir(p.alyze, "globSA+.+1kmrect_sum.tif")))) {
  ...
  globsa_list_sum <- lapply(dir(p.alyze, "globSA+.+1kmrect_sum.tif")[c(2:3, 1)], raster)
} 
